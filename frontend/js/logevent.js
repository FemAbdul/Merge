...
function handleLogin(event) {
    ...
    .then(response => response.json().then(data => {
        if (response.status === 403 && data["2fa_required"]) {
            // 2FA Required – Store username and show OTP input
            const currentUser = { username: data.username };
...
// Function to handle the 2FA prompt
function show2FAPrompt() {
    const content = document.getElementById('content');
    content.innerHTML = `
        <div id="2faPrompt" class="page">
            <div class="row justify-content-center align-items-center" style="height:80vh;">
                <div class="col-md-4">
                    <h3 class="text-center">Enter OTP Code</h3>
                    <form id="2faForm">
                        <div class="mb-3">
...
// Function to handle OTP code verification
function handle2FAVerification(event) {
    ...
    if (!otpCode || !currentUser?.username) {
        document.getElementById('errorMessage').textContent = "Invalid OTP or user session.";
        return;
    }
    console.log("current user in verify2fa:",currentUser);
    // Make a POST request to the verify 2FA endpoint
    ...
    .then(data => {
        // Successful 2FA verification – Update user with tokens and other details
        // currentUser = { ...data };
        saveUserAndRedirect(data);
    })
    .catch(error => {
...
// Function to save user details and redirect to dashboard
